...
export class Client {
  private readonly starsIndices: Set<number>;
  private readonly starsMap: Map<number, Point>;
  /**
   ...
   */
  public constructor(private readonly puzzle: Puzzle) {
  ...
  private checkRep(): void {
    for (const index of this.starsIndices) {
      assert(
        index >= 0 && index < this.puzzle.rows * this.puzzle.cols,
        "index in starsIndices out of bounds"
      );
      assert(
        this.starsMap.has(index),
        "all elements in starsIndices must be a key in starsMap"
      );
    }
    for (const key of this.starsMap.keys()) {
      assert(
        this.starsIndices.has(key),
        "all keys in starsMap must be in starsIndices"
      );
    }
  }
   ...
   */
  private getIndex(star: Point): number {
    assert(
      star.row > 0 &&
        star.col > 0 &&
        star.row <= this.puzzle.rows &&
        star.col <= this.puzzle.cols,
      "star position put of bounds"
    );
    assert(
      star.region >= 0 && star.region < this.puzzle.rows,
      "star region out of bounds"
    );
    const starIndex = (star.row - 1) * this.puzzle.cols + (star.col - 1);
    assert(
      starIndex >= 0 && starIndex < this.puzzle.rows * this.puzzle.cols,
   ...
   *          false otherwise
   */
  public addStar(star: Point): boolean {
    this.checkRep();
    if (this.isSolved()) {
      return false;
    }
    ...
    if (rowCount > 2 || colCount > 2 || regionCount > 2) {
      throw new Error(
        "there are already more than 2 stars in this row, col, region"
      );
   ...
   */
  public removeStar(star: Point): boolean {
    this.checkRep();
    const starIndex = this.getIndex(star);
